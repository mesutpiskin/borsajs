...
export class DovizcomProvider extends BaseProvider {
    ...
    private static readonly SUPPORTED_ASSETS = new Set([
        ...
        'gram-altin', 'gumus', 'ons',
        'XAG-USD', 'XPT-USD', 'XPD-USD',
        'BRENT', 'WTI',
        'diesel', 'gasoline', 'lpg',
    ]);
    ...
    private async getToken(): Promise<string> {
        ...
        try {
            const response = await this.get<string>('https://www.doviz.com/', {
    ...
    private getOrigin(asset: string): string {
    ...
    async getCurrent(asset: string): Promise<FXCurrentData> {
        // Normalize asset
        const normalizedAsset = DovizcomProvider.SUPPORTED_ASSETS.has(asset.toUpperCase())
            ? asset.toUpperCase()
            : asset;

        if (!DovizcomProvider.SUPPORTED_ASSETS.has(normalizedAsset)) {
        ...
        try {
            let data: ArchiveItem | null;

            if (DovizcomProvider.FUEL_ASSETS.has(normalizedAsset)) {
                data = await this.getFromArchive(normalizedAsset, 7);
            } else {
                data = await this.getFromDaily(normalizedAsset);
            }
            ...
            const result: FXCurrentData = {
                symbol: normalizedAsset,
                last: Number(data.close) || 0,
                open: Number(data.open) || 0,
                high: Number(data.highest) || 0,
                low: Number(data.lowest) || 0,
                updateTime: data.update_date ? new Date(data.update_date * 1000) : undefined,
            };
    ...
    async getHistory(asset: string, options: HistoryOptions = {}): Promise<FXHistoryData[]> {
        ...
        if (start) {
            ...
            const days = DovizcomProvider.PERIOD_DAYS[period] ?? 30;
        ...
        try {
            ...
            const url = `${DovizcomProvider.BASE_URL}/assets/${normalizedAsset}/archive`;
            const response = await this.get<DovizApiResponse>(url, {
                params: {
            ...
            const archive = response.data?.data?.archive ?? [];

            const records: FXHistoryData[] = archive.map(item => ({
                date: new Date((item.update_date ?? 0) * 1000),
                open: Number(item.open) || 0,
                high: Number(item.highest) || 0,
                low: Number(item.lowest) || 0,
            ...
            throw new APIError(`Failed to fetch history for ${asset}: ${error}`);
        }
    }

    private async getFromDaily(asset: string): Promise<ArchiveItem | null> {
        const token = await this.getToken();
        const origin = this.getOrigin(asset);

        const url = `${DovizcomProvider.BASE_URL}/assets/${asset}/daily`;
        const response = await this.get<DovizApiResponse>(url, {
            params: { limit: 1 },
            headers: {
                'Authorization': `Bearer ${token}`,
    ...
    private async getFromArchive(asset: string, days: number): Promise<ArchiveItem | null> {
        ...
        const url = `${DovizcomProvider.BASE_URL}/assets/${asset}/archive`;
        const response = await this.get<DovizApiResponse>(url, {
 ...
 */
export function getDovizcomProvider(): DovizcomProvider {
    if (!provider) {
        provider = new DovizcomProvider();
    }
    return provider;
}
